...
library(dplyr) 
root <- getwd()
hpc_root <- "/sc/arion/projects/pd-omics/brian/COVID19_GWAS"
```
...
sumstats_paths <- lapply(1:nrow(DAT), function(i){
  ROW <- DAT[i,]
  # printer(ROW$SNP) 
  out <- file.path("data/sumstats",ROW$SNP,paste0(ROW$SNP,"_COVID19.tsv"))
...
```{r} 
GWAS.QTL <- data.table::fread(file.path(root,"catalogueR_queries/eQTL_Catalogue.tsv.gz"), nThread = 4)
```
...
counts <- GWAS.QTL %>% 
  ...
  dplyr::summarise(variants=dplyr::n_distinct(SNP),
                   eVariants=dplyr::n_distinct(SNP[pvalue.QTL<sig_thresh]),
                   genes=dplyr::n_distinct(gene.QTL),
                   eGenes=dplyr::n_distinct(gene.QTL[pvalue.QTL<sig_thresh]),
                   eGenes_names=paste(unique(gene.QTL[pvalue.QTL<sig_thresh]), collapse = ", "),
...
gp <- ggplot(data = subset(GWAS.QTL, pvalue.QTL< sig_thresh), 
       aes(x= gene.QTL, y= Tissue, color=P, size= pvalue.QTL)) +
  geom_point(alpha=1) + 
  ...
  labs(title = "Variant overlap: COVID-19 GWAS vs. eQTL Catalogue",
       subtitle = paste("QTL p-value <",sig_thresh)) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        strip.background = element_rect(fill="grey20"),
        strip.text = element_text(color="white"))
print(gp)
ggsave("plots/COVID19_GWAS.summary.png", gp, dpi=300)
```
